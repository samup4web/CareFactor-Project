...
class Tank_auth {
    ...
    function logout()
    {
        ...
        $this->ci->session->set_userdata(array('user_id' => '', 'email'=>'', 'username' => '', 'status' => ''));
    ...
    function create_user($username, $email, $password, $email_activation, $userInfo)
    {
        ...
        } else
        {
            ...
            {
                $create_user_success_flag = true;
                $temp_user_id = $res['user_id'];
                $this->ci->users->update_profile_info((int)$res['user_id'], $userInfo['user_type']);
